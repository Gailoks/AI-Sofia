...
class AI:
    ...
    def create_new_nnmodel(self):
        self.model = NNModule(self.vocabulary_size + AI.SERVICE_TOKENS, 1000, 500, 2, self.vocabulary_size + AI.SERVICE_TOKENS).to(self.device)
...
class NNModule(nn.Module):
    ...
    def bind(self, ai:AI):
        self.__ai = ai
        self.__end_tensor = torch.LongTensor([self.__ai.get_service_token(AI.ST_END)]).to(self.__ai.device)
    ...
    def evaluate(self, text:str, prediction_lim:int) -> str:
        text_tensor = torch.LongTensor(list(self.__ai.tokenizer.tokenize(text))).to(self.__ai.device)
        hidden = self.__init_hidden()
        predicted_text = StringBuilder()

        for _ in range(prediction_lim):
            next_w, hidden = self(text_tensor.view(-1, 1).to(self.__ai.device), hidden)
